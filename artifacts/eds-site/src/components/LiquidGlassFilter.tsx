// Singleton SVG feDisplacementMap filter for the Glass Float effect.
// Pre-computes a 192×192 SDF displacement map once; attach/detach are called
...
const roundedRectSDF = (x: number, y: number, w: number, h: number, r: number): number => {
...
let _mapURL:  string | null                       = null;

function buildDisplacementMap(): string {
  const canvas = document.createElement("canvas");
  canvas.width = canvas.height = MAP_RES;
...
/** Snap the filter to `el` and apply the liquid-glass backdropFilter. */
export function attachLiquidGlass(el: HTMLElement): void {
  ...
  _feDisp.setAttribute("scale",  String(Math.max(14, Math.min(r.width, r.height) * 0.15)));

  // Applied to all floatable surfaces. On .glass/.glass-elevated the
  // refraction shows through the semi-transparent fill; on solid-bg buttons
  // it is visible at border-radius corners and in the elevated shadow halo.
  ...
  return (
    ...
    >
      <defs>
        ...
        >
          <feImage ref={feImgRef} width="100" height="100" result={`${FILTER_ID}_map`} />
          <feDisplacementMap
            ref={feDispRef}
            in="SourceGraphic"
            in2={`${FILTER_ID}_map`}
